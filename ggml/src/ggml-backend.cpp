...
static void ggml_backend_sched_copy_inputs(ggml_backend_sched_t sched, ggml_backend_sched_split * split, std::array<bool, GGML_SCHED_MAX_BACKENDS> & needs_sync,
        std::vector<int32_t> & ids, std::vector<ggml_bitset_t> & used_ids, ggml_tensor * & last_ids_tensor) {
    ...
    for (int j = 0; j < split->n_inputs; j++) {
        ...
        if (input->flags & GGML_TENSOR_FLAG_INPUT) {
            ...
            if (sched->only_active_experts && split->graph.n_nodes > 0 &&
                ...
                int64_t n_expert = node->src[0]->ne[2];
                ...
                if (input->ne[2] > 1) {
                    ...
                    auto next_on_id = [&used_ids, n_expert] (int id) {
                        while (id < n_expert && !ggml_bitset_get(used_ids.data(), id)) ++id;
                        return id;
                    };
                    auto next_off_id = [&used_ids, n_expert] (int id) {
                        while (id < n_expert && ggml_bitset_get(used_ids.data(), id)) ++id;
                        return id;
                    };
                    ...
                    size_t copy_size = ggml_nbytes(input);
...
static enum ggml_status ggml_backend_sched_eval(ggml_backend_sched_t sched, ggml_backend_t split_backend, struct ggml_backend_sched_split * split);

// simple reusable barrier for async scheduling (C++11 compatible)
class ggml_backend_sched_barrier {
public:
    ggml_backend_sched_barrier(int n) : count(n), expected(n), phase(0) {}
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    // async scheduling path: overlap reduce/copy/compute across backends
    if (sched->scheduler_async && sched->n_backends > 2 && sched->split_mode_graph && sched->has_reduce) {
        ...
        ggml_backend_sched_barrier barrier(sched->n_backends);
        auto compute = [sched, &barrier, first_reduce] (int ith) {
            ...
            for (int i = 0; i < sched->n_splits; i++) {
                ...
                if (split->n_inputs > 0) {
                    ...
                    if (ith == copy_thread) {
                        ggml_backend_sched_copy_inputs(sched, split, sched->needs_sync, ids, used_ids, last_ids_tensor);
                    }
                    barrier.arrive_and_wait();
                }
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        ggml_backend_sched_copy_inputs(sched, split, sched->needs_sync, ids, used_ids, prev_ids_tensor);
...
ggml_backend_sched_t ggml_backend_sched_new(
    ...
    sched->workers.reserve(n_backends);
    sched->statuses.resize(n_backends, GGML_STATUS_SUCCESS);
...
static void ggml_sched_prepare_graph(ggml_backend_sched_t sched) {
    ...
    if (sched->split_mode_graph) {
        ...
        for (int backend_id = 0; backend_id < sched->n_backends; ++backend_id) {
            if (ggml_backend_dev_type(ggml_backend_get_device(ggml_backend_sched_get_backend(sched, backend_id))) == GGML_BACKEND_DEVICE_TYPE_CPU) continue;
